 ...
 *  2. Floating parallax orbs reacting to mouse + scroll
 *  3. Scroll-reveal (fade-up + stagger) via IntersectionObserver
 *  4. Magnetic hover on .btn-primary, .btn-glow, .wa-btn
 *  5. Counter animation on stats (99.98%, <50ms, SOC 2, etc.)
 *  6. Hero headline word-split entrance
 *  7. Navbar hide/show on scroll direction
 *  8. Card tilt 3D on hover (bento cards)
 *  9. Scroll progress bar at top of page
 * 10. Video scanline effect
 */
...
(function () {
  ...
  /**
   * Waits for Babel standalone to finish compiling JSX and React to finish
   * the initial render. Uses MutationObserver on #root so it reacts the
   * instant the sentinel selector appears, rather than waiting a fixed delay.
   * Resolves after `timeout` ms regardless, so animations are never blocked
   * permanently if rendering stalls.
   */
  function waitForBabelRender(selector, timeout) {
  ...
  function initScrollReveal() {
    ...
    style.textContent = `
      .ax-reveal { opacity: 0; transform: translateY(28px); transition: opacity 0.75s cubic-bezier(0.16, 1, 0.3, 1), transform 0.75s cubic-bezier(0.16, 1, 0.3, 1); }
      .ax-reveal.ax-visible { opacity: 1; transform: translateY(0); }
      .ax-reveal-left { opacity: 0; transform: translateX(-32px); transition: opacity 0.8s cubic-bezier(0.16, 1, 0.3, 1), transform 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
      .ax-reveal-left.ax-visible { opacity: 1; transform: translateX(0); }
      .ax-reveal-scale { opacity: 0; transform: scale(0.94); transition: opacity 0.7s cubic-bezier(0.16, 1, 0.3, 1), transform 0.7s cubic-bezier(0.16, 1, 0.3, 1); }
      .ax-reveal-scale.ax-visible { opacity: 1; transform: scale(1); }
    `;
  ...
  function initCounters() {
    // Target the hero stats: 99.98%, <50ms, SOC 2.
    // React renders fontSize:20 as font-size:20px (kebab-case) in the DOM.
    const statEls = document.querySelectorAll('.mono[style*="font-size: 20px"]');
  ...
  /* ──────────────────────────────────────────────
   * 7. HERO HEADLINE WORD SPLIT
   * ────────────────────────────────────────────── */
  function initHeroHeadline() {
    const style = document.createElement('style');
    style.textContent = `
      @keyframes ax-word-in {
        from { opacity: 0; transform: translateY(24px) rotateX(-15deg); filter: blur(4px); }
        to   { opacity: 1; transform: translateY(0) rotateX(0deg); filter: blur(0px); }
      }
      .ax-word { display: inline-block; animation: ax-word-in 0.7s cubic-bezier(0.16, 1, 0.3, 1) both; }
    `;
    document.head.appendChild(style);
    ...
    // Split text nodes only, preserve child elements (gradient-text spans)
  ...
  function initNavbar() {
    const style = document.createElement('style');
    style.textContent = `
      .nav { transition: transform 0.4s cubic-bezier(0.16, 1, 0.3, 1), background 0.3s ease, box-shadow 0.3s ease !important; }
      .nav.ax-nav-hidden { transform: translateY(-100%) !important; }
      .nav.ax-nav-scrolled { background: rgba(5,7,13,0.92) !important; box-shadow: 0 1px 0 rgba(34,211,238,0.08) !important; }
    `;
    document.head.appendChild(style);
  ...
  /* ──────────────────────────────────────────────
   * 11. GRID SCANLINE EFFECT ON HERO VIDEO
   * ────────────────────────────────────────────── */
  function initVideoScanline() {
    const style = document.createElement('style');
    style.textContent = `
      @keyframes ax-scanline {
      ...
      .hero-video-inner::after {
        ...
        animation: ax-scanline 4s ease-in-out infinite;
        pointer-events: none;
        z-index: 2;
      }
    `;
  ...
  function initCTAGlow() {
    const style = document.createElement('style');
    style.textContent = `
      @keyframes ax-border-pulse {
        0%, 100% { box-shadow: 0 0 0 0 rgba(34,211,238,0.0), 0 20px 60px -20px rgba(0,0,0,0.5); }
        50%       { box-shadow: 0 0 30px -8px rgba(34,211,238,0.15), 0 20px 60px -20px rgba(0,0,0,0.5); }
      }
      #contacto > .container > div:first-child {
        animation: ax-border-pulse 4s ease-in-out infinite;
      }
    `;
    document.head.appendChild(style);
  }
   ...
   * ────────────────────────────────────────────── */
  onReady(() => {
    // These create their own DOM elements or inject CSS — no React DOM needed.
    initCursor();
    initOrbs();
    initScrollProgress();
    initNavbar();
    ...
    // Wait for Babel standalone to compile app.jsx / tweaks-panel.jsx and
    // React to complete the initial render before querying any
    // React-rendered selectors. We use .bento as the sentinel because it is
    // one of the last top-level sections rendered; by the time it exists,
    // h1, .nav, .card, .btn-primary, .btn-glow, .wa-btn, .marquee-track,
    // and .hero-video-inner are all guaranteed to be in the DOM.
    waitForBabelRender('.bento').then(() => {
      initHeroHeadline();
      initScrollReveal();
      initMagneticButtons();
      initCardTilt();
